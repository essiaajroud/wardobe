...
VALID_STRUCTURES = [
    ...
    ["Top", "Skirt", "Coat"],
    ["Dress_Casual"],         # Changé
    ["Dress_Wedding"],        # Ajouté
    ["Dress_Casual", "Coat"],  # Changé
    ["Dress_Wedding", "Coat"] 
]
...
class RLRecommender:
    ...
    def recommend_with_rl(self, wardrobe, encoder, scaler, num_features, cat_features,
        ...
        for outfit in all_outfits:
            ...
            final_score = base_score * (1 + rl_adjustment)
...
def outfit_to_vector(outfit, base_model):
    ...
    has_dress = int(any("Dress" in i["type"] for i in outfit))
        ...
        for temp in [5, 15, 25, 35]:
            for occ in OCCASIONS:
                rows.append({
                    ...
                    "warmth": np.mean(warmths),
                    "breath": np.mean(breaths),
                    "has_dress": int(any("Dress" in i["type"] for i in o)),
